...
from planner_MCTS import MonteCarloTreeSearch
...
def run(rounds_num:int, config_path:str, save_path:str, no_animation:bool, save_fig:bool) -> None:
    ...
    for iter in range(rounds_num):
        ...
        while True:
            ...
            for vehicle in vehicles:
                vehicle_cur_action,vehicle_expected_traj= vehicle.execute(vehicles.exclude(vehicle))
                vehicle.expected_traj = vehicle_expected_traj
                vehicle.cur_action = vehicle_cur_action
                if not vehicle.is_get_target:
                    vehicle.state = kinematic_propagate(vehicle.state, vehicle_cur_action.value, delta_t)
            ...
            if not no_animation:
                ...
                env.draw_env()
                for vehicle in vehicles:
                    expected_traj = vehicle.expected_traj.to_list()
                    vehicle.draw_vehicle(True)
                    plt.plot(vehicle.target.x, vehicle.target.y, marker='x', color=vehicle.color)
                    plt.plot(expected_traj[0], expected_traj[1], color=vehicle.color, linewidth=1)
...
if __name__ == "__main__":
    ...
    if args.config is None:
        config_file_path = os.path.join(os.path.dirname(os.path.dirname(current_file_path)),'MCTS',
                                        'config', 'triple_interact.yaml')
    else:
